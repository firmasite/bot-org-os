...
## Preferred path — paste INSTALL-PROMPT

Copy [`INSTALL-PROMPT.md`](../INSTALL-PROMPT.md) into any new Bot that can create agents and install Skills.

That Bot will **become Chief**. It reads [`AGENTS.md`](../AGENTS.md) and runs [`BOOTSTRAP.md`](../BOOTSTRAP.md). Pasting INSTALL-PROMPT is the install trigger. A bare GitHub link is not enough. Do not CreateAgent a duplicate Chief. Success = this Bot is Chief, four specialists, Skills, playbooks on the bus, Core Team channel, `workspace/org/INSTALL-STATUS.md`, and a beginner welcome (Chief introduces themself + what exists + Pin tip).
